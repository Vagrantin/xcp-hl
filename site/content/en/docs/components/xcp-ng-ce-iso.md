...
[`xolite-ce`](/docs/components/xolite-ce) and the signed `xoa-proxy` RPM produced by
[`xoa-proxy`](/docs/components/xoa-proxy), overlays them onto a stock XCP-ng 8.3 base using the
...
### host-installer patching for the ISO storage partition {#host-installer-patching}
