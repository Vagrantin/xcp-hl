...
[xcp-ng-ce-iso](/docs/components/xcp-ng-ce-iso#host-installer-patching).

## August 2026

### Release matrix states the exact package shipped, towards [#15](https://github.com/Vagrantin/xcp-hl/issues/15)

The [Release Matrix](/docs/reference/release-matrix) recorded only a release tag per
...
ISOs keep resolving them; the [Release Matrix](/docs/reference/release-matrix#xoa-hl-releases)
...
The [Release Matrix](/docs/reference/release-matrix) now has a dedicated
...
- xolite-ce `v0.21.0-ce6`: upstream xo-lite pinned to `0.21.0`, the last
  known-good release (`0.22.0` and `0.23.0` broke the build), through the
  `UPSTREAM_TAG` file.
- xoa-proxy `v0.1.1.x`: simpler RPM versioning (`.static` release suffix)
...
### First public release, v8.3-ce alpha2

- First usable release.
- Deploys XOA from the Vates image, Ronivay's image, or a custom URL.
...
- ISO assembled from upstream XCP-ng 8.3 with a community RPM repository
  layered on top.
