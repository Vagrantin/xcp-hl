...
XCP-ng**, avec le RPM [`xoa-hl`](/docs/components/xoa-hl) à l'intérieur. Le résultat est
...
- Une release [`xoa-hl`](/docs/components/xoa-hl) publiée sur GitHub (le RPM de la
...
4. Publie la XVA comme release GitHub taguée `xoa-image-<date>-<sha7>` sur
   **`Vagrantin/build-xoa-hl`**, ce dépôt (`<sha7>` est le commit `xoa-hl` à
   ...
   [matrice des versions](/docs/reference/release-matrix#xoa-hl-releases).
