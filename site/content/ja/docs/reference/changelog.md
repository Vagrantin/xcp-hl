...
[リリース一覧表](/docs/reference/release-matrix)にあります。
{class="lead"}
...
実現しています。
[xcp-ng-ce-iso](/docs/components/xcp-ng-ce-iso#host-installer-patching) を
参照してください。
...
[リリース一覧表](/docs/reference/release-matrix)には、コンポーネントごとのリリース
...
[リリース一覧表](/docs/reference/release-matrix#xoa-hl-releases)では、各項目を
それを置いているリポジトリにリンクしています。

### リリース一覧表に XOA のバージョンを記録。[#13](https://github.com/Vagrantin/xcp-hl/issues/13) を修正

[リリース一覧表](/docs/reference/release-matrix)に、**XOA アプライアンスのリリース**
...
- ISO のビルドはオーケストレーターが実行し、コンポーネントの正確なリリース
  ...
  `68f211d`）。
- すべての ISO のリリースを[リリース一覧表](/docs/reference/release-matrix)に記録して
  います。
...
[GPG 署名](/docs/components/#gpg-signing)を参照してください。さらに細かくする案
（モジュールごとに 1 つの鍵）は
