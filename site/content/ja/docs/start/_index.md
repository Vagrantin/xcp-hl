...
オープンソースのハイパーバイザーです（ハードウェア上で直接動作し、仮想マシン
を動かすソフトウェアです。「タイプ 1」と呼ばれる形式にあたります）。公式版
には、ブラウザーで動く
...
コミュニティが保守するワークフローに置き換えます。XOA を展開する方法を
次の 4 通りから選べます。
...
含まれる正確なバージョンは[リリース一覧表](/docs/reference/release-matrix)に
記録しています。
