...
AlmaLinux 9 の VM に、[`xoa-hl`](/docs/components/xoa-hl) の RPM を入れたものです。
...
- GitHub で公開済みの [`xoa-hl`](/docs/components/xoa-hl) のリリース（最新リリースの
  RPM は自動で解決します）。
...
4. XVA を、**`Vagrantin/build-xoa-hl`**（このリポジトリ）の
   ...
   [リリース一覧表](/docs/reference/release-matrix#xoa-hl-releases)に記録します。
