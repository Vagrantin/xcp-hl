...
このリポジトリは、[`xolite-ce`](/docs/components/xolite-ce) が作った署名済みの
`xo-lite-ce` RPM と、[`xoa-proxy`](/docs/components/xoa-proxy) が作った署名済みの
...
### ISO ストレージのパーティションのための host-installer へのパッチ {#host-installer-patching}
